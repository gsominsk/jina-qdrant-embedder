...
Когда сервисы запущены, но не обрабатывают запросы (т.е. индексация не происходит), их базовое потребление памяти выглядит следующим образом:

-   **Сервис эмбеддингов (Jina):** ~921 МиБ
-   **База данных Qdrant:** ~261 МиБ

Это объем памяти, необходимый для загрузки моделей и поддержания сервисов в готовом состоянии. Потребление памяти будет увеличиваться во время активной индексации, как описано в разделе о производительности.
