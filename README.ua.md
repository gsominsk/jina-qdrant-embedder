...
## Підсумки та продуктивність

Після інтенсивного налагодження та оптимізації користувацький сервіс ембедингів тепер повністю стабільний та продуктивний.

### Ключові досягнення
- **Усунення витоків пам'яті:** Два незалежні витоки пам'яті (один пов'язаний з тензорами PyTorch, інший — з життєвим циклом запитів FastAPI/Starlette) були виявлені та повністю усунені.
- **Стабільне споживання пам'яті:** Під високим навантаженням споживання пам'яті сервісом стабільне, досягаючи піка приблизно в **4.4 ГБ** і коливаючись у безпечному діапазоні **3-5 ГБ**. Це запобігає збоям через брак пам'яті (OOM).
- **Висока пропускна здатність:** Сервіс здатний обробляти велику кількість одночасних запитів, забезпечуючи швидке та надійне створення ембедингів.
...
#### Що таке семафор?
Семафор, визначений у `embeddings/app/app.py`, обмежує кількість одночасних запитів, які можуть оброблятися моделлю ембедингів у будь-який момент часу. Це запобігає перевантаженню системи та вичерпанню пам'яті.
...
#### Як керувати семафором
`SEMAPHORE_VALUE` — це найважливіший параметр для налаштування продуктивності.

- **Поточне значення:** Оптимальне значення було визначено як **8**. Це забезпечує найкращий баланс між пропускною здатністю та стабільністю пам'яті на системі з 8 ГБ доступної оперативної пам'яті.
- **Коли його змінювати:** Вам слід змінювати це значення лише в тому випадку, якщо ви запускаєте сервіс на машині зі значно більшим або меншим об'ємом оперативної пам'яті.
  ...
  - **Менше RAM:** Якщо ви стикаєтеся зі збоями OOM на машині з меншим об'ємом пам'яті, ви **повинні** зменшити це значення (наприклад, до `4` або `6`). Це знизить навантаження на систему.
- **Як його змінити:** Відредагуйте константу `SEMAPHORE_VALUE` безпосередньо у файлі [`embeddings/app/app.py`](embeddings/app/app.py:1) та перезапустіть сервіс.

## Споживання ресурсів (у простої)

Коли сервіси запущені, але не обробляють запити (тобто індексація не відбувається), їх базове споживання пам'яті є таким:

-   **Сервіс ембедингів (Jina):** ~921 МіБ
-   **База даних Qdrant:** ~261 МіБ

Це обсяг пам'яті, необхідний для завантаження моделей та підтримки сервісів у готовому стані. Споживання пам'яті буде збільшуватися під час активної індексації, як описано в розділі про продуктивність.
